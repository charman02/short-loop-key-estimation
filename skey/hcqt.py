...
class CropCQT(torch.nn.Module):
    ...
    def forward(self, spectrograms: torch.Tensor, transpose: torch.Tensor) -> torch.Tensor:
        cropped = []
        for s, start_idx in zip(spectrograms, transpose):
            start = int(start_idx)
            # Clamp so crop fits within bounds
            if start + self.height > s.shape[1]:
                start = max(0, s.shape[1] - self.height)
            end = start + self.height
            cropped.append(s[:, start:end, :])
    
        if len(cropped) == 0:
            raise ValueError("All crops were skipped — check your inputs or crop indices.")
