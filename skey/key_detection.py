...
DEFAULT_CHECKPOINT_PATH = Path(__file__).parent / "models/ks_mode/7/basic/skey/skey.pt"
...
def load_audio(song_path: str, sr: float, audio_len: int, mono: bool = True, normalize: bool = True) -> torch.Tensor:
    """
    ...
    try:
        waveform, waveform_sr = torchaudio.load(song_path)
    ...
    # Pad or trim
    if waveform.size(1) < audio_len:  # time dimension
        waveform = F.pad(waveform, (0, audio_len - waveform.size(1)))
    elif waveform.size(1) > audio_len:
        waveform = waveform[:, :audio_len]  # trim
