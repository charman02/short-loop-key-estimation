...
def restart_from_checkpoint(
    ...
    if os.path.isfile(ckpt_path):
        ...
        print("Fine-tuning mode: loading model weights only.")
        # Only load model parameters
        if "stone" in checkpoint:
            model.stone.load_state_dict(checkpoint["stone"])
        # Reset training state in save_dict
        save_dict["epoch"] = 0
        save_dict["val_loss"] = float("inf")
