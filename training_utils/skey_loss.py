...
class CrossPowerSpectralDensityLoss(nn.Module):
    """
    Differentiable distance on the circle of fifths.
    """
    ...
    def forward(self, y: torch.Tensor) -> Dict[str, Union[int, float, Dict]]:
        ...
        # for 3 views
        m1_source1 = m1[:batch_size].unsqueeze(1)
        m1_source2 = m1[batch_size : 2 * batch_size].unsqueeze(1)
        m1_equivariant = m1[2 * batch_size :].unsqueeze(1)
        ...
        # Label the data by the cqt bin energy comparison
        with torch.no_grad():
            key_signature = ((y[:batch_size] + y[batch_size : 2 * batch_size]) / 2).argmax(dim=1)
            key_signature = key_signature.clamp(0, 11)  # Ensure it's in range
        
            mean_hcqt = mean_hcqt.unsqueeze(2)  # [B, 12, 1]
            key_signature_expanded = key_signature.view(-1, 1, 1)  # [B, 1, 1]
            
            majorminor = torch.gather(mean_hcqt, 1, key_signature_expanded).squeeze(2)  # [B, 1] -> [B]
